...
public enum DiscontinuityDir: Sendable {
    case dropaway   // Ground drops (depth increases suddenly) — curb, stairs down, hole
    case riseup     // Ground rises (depth decreases suddenly) — step up, ramp, obstacle base
}

public struct Discontinuity: Sendable {
    public let column: Int              // Which horizontal column (0 to columns-1)
    public let bearing: Float           // Radians from center
    public let relativeDepth: Float     // Raw depth value where it occurs
    public let magnitude: Float         // Size of the depth jump (0-1 normalized)
    public let direction: DiscontinuityDir

    public init(column: Int, bearing: Float, relativeDepth: Float, magnitude: Float, direction: DiscontinuityDir) {
        self.column = column
...
public struct SceneUnderstanding: Sendable {
    public let columns: Int
    public let traversability: [Float]          // Per-column, 0=blocked 1=clear
    public let obstacleDistance: [Float]         // Per-column, nearest non-walkable depth
    public let discontinuities: [Discontinuity] // Ground-plane breaks detected
    public let groundPlaneRatio: Float          // Overall walkable ground fraction (0-1)
    public let columnBearings: [Float]          // Bearing in radians for each column
}

// MARK: - Analyzer

public final class SceneAnalyzer {
    public var columns: Int = 36
